...
def filter_data(dataframe, num_period):
    """Filter DataFrame by time range. Handles cases where Date is in index."""
...
def close_chart(dataframe, num_period=False):
    ...
    fig.update_layout(height=500, margin=dict(l=0, r=20, t=0, b=0),
...
def Moving_average_forecast(dataframe, num_period):
...
def MACD(dataframe, num_period):
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=dataframe['Date'], y=dataframe['MACD'], name='MACD', line=dict(width=2, color='red', dash='dash')))
    fig.add_trace(go.Scatter(x=dataframe['Date'], y=dataframe['MACD Signal'], name='Signal', line=dict(width=2, color='blue')))
    fig.add_trace(go.Bar(x=dataframe['Date'], y=dataframe['MACD Hist'], name='Histogram', marker_color='gray', opacity=0.3))
...
#     # Prepare alternating colors for each row
#     fill_colors = []
#     for _ in range(len(dataframe.columns) + 1):  # +1 for index column
#         fill_colors.append(
#             [rowOddColor if i % 2 == 0 else rowEvenColor for i in range(len(dataframe))]
#         )
...
#             values=[dataframe.index] + [dataframe[col] for col in dataframe.columns],
#             line_color='white',
#             fill_color=fill_colors,  # ✅ correctly dimensioned color matrix
#             align='left',
#             font=dict(color='black', size=14),
#             height=28
#         )
...
#     fig.add_trace(go.Scatter(x=dataframe['Date'], y=dataframe['Low'],mode="lines",name='Low',line = dict(width=2,color = 'red')))
#     fig.update_xaxes(rangeslider_visible=True)
#     fig.update_layout(height = 500,margin=dict(l=0, r=20, t=0, b=0), plot_bgcolor = 'white',paper_bgcolor ='#e1efff',legend=dict(yanchor="top",xanchor="right"))
#     return fig
...
#     fig.add_trace(go.Scatter(
#         x=dataframe['Date'],
#         y=dataframe['MACD'], name = 'MACD',marker_color='orange',line = dict(width=2,color = 'red',dash='dash'),
#     ))
