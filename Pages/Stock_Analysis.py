...
datal = ticker_.history(period = 'max')
if num_period == '':

    if chart_type == 'Candle' and indicators == 'RSI':
        st.plotly_chart(candlestick(datal, '1y'), use_conatainer_width=True)
        st.plotly_chart(RSI(datal, '1y'), use_container_width=True)

    if chart_type == 'Candle' and indicators == 'MACD':
        st.plotly_chart(candlestick(datal, '1y'), use_container_width=True)
        st.plotly_chart(MACD(datal, '1y'), use_container_width=True)

    if chart_type == 'Line' and indicators == 'RSI':
        st.plotly_chart(close_chart(datal, '1y'),use_container_width=True)
        st.plotly_chart(RSI(datal, '1y'), use_container_width=True)

    if chart_type == 'Line' and indicators == 'Moving Average':
        st.plotly_chart(Moving_average_forecast(datal, '1y'), use_container_width=True)
    ...
    if chart_type == 'Candle' and indicators == 'MSI':
    ...
    if chart_type == 'Line' and indicators == 'Moving Average':
        st.plotly_chart(Moving_average_forecast(new_df1, num_period), use_container_width=True)
    if chart_type == 'Line' and indicators == 'MACD':
        st.plotly_chart(close_chart(new_df1, num_period), use_container_width=True)
        st.plotly_chart(MACD(new_df1, num_period), use_container_width=True)  
